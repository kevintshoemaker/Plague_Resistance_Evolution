...
if(ELIZABETHOFFICE) rootDir <- "C:\\Users\\ehunter\\Dropbox\\Nevada\\PlagueModeling\\"
if(ELIZABETH) rootDir <- "~/Dropbox/Nevada/PlagueModeling/"

ScriptDir <- paste(rootDir,"Rscript",sep="")
DataDir <- paste(rootDir,"PrairieDogData",sep="")
CovDir <- paste(rootDir,"Covariates",sep="")
MODISDir <- paste(CovDir, "MODIS", sep="\\")
...
#Random Forest (party) object
setwd(DataDir)
load("randomForestModel_2017-05-12.RData")
...
########
#EXAMPLE CODE for creating rasters of "internal" covariates and state variable
#Reading in Pawnee data
...
load("MasterColonies_2017-05-09.RData")

#List of shapefiles for given site
fgdb_list_site <- fgdb_list[substr(fgdb_list, 1, 5) == "Pawne"] 
#Surveyed years
site.years <- as.numeric(str_sub(fgdb_list_site, -4,-1))

#DATA PROCESSING (the next three chunks are just what is needed to create the internal covariates)
#Read site shapefiles into the workspace and convert to raster
#Create blank raster (10m resolution)
r <- raster()
extent(r) <- extent(tmax) #Using smaller extent from covariates (snippet of Pawnee)
res(r) <- c(100,100)
Pawn <- stack(r) 
...
mcP <- crop(master.colony.Pawne, extent(r))
plagued <- stack(r)
...
####INTERNAL COVARIATES
#OCCUPIED AREA using the master colony dataset, using the previous year's size as the raster field
area <- stack(r)
for (i in 1:(nlayers(Pawn)-1)){
	temp <- rasterize(mcP, r, field=mcP@data[,i+1])
	mask.c <- status[[i]]
...
for (i in 1:(nlayers(Pawn)-1)){
	...
	cells.patch <- freq(temp.patch)
	cells.patch <- as.data.frame(cells.patch[1:(nrow(cells.patch)-1),])
	temp.psize <- subs(temp.patch, cells.patch, by=1, which=2)
...
slope.trans <- transition(slope.c, mean, 8)
slope.cost.plague <- stack(r)
dist.noplague <- Pawn[[1]]
values(dist.noplague) <- 500000 #Need a raster for distance if there's no plague in the area (put distance at 500km)
